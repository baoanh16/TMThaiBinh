...
lang['vi'] = {
	...
	'searchResultName': '',
	'fax': 'Fax',
	'phoneMaintenance': 'Bảo trì sữa chữa',
	'hotline': 'Hotline',
	'phone': 'Điện thoại',
	'address': 'Địa chỉ',
	'zoom': 'Phóng to',
...
$(document).ready(function () {
	//nếu không có quốc gia 
	setTimeout(function () {
		$("#ddlCountry").val("99f791e7-7343-42e8-8c19-3c41068b5f8d").trigger("change");
	}, 1000);
						...
						if (results.length > 0) {
							...
							jQuery.each(results, function (k, v) {
								...
								if (v.getAttribute("phone") != "") {
									phoneFax += "<div class='phone'><span>" + lang[currCulture].phone + ":</span> " + v.getAttribute("phone") + "</div>";
								}
								...
								if (number > 9) {
									$("<li id='l_" + marker['__gm_id'] + "' class='clinic_list double-digit' />")
										...
										.click(function () {
											...
											zoomHere(marker.getPosition().lat(), marker.getPosition().lng());
											$('.btn-mapmenu').trigger('click');
										})
										.appendTo("#list");
								} else {
		...
		success: function (msg) {
			...
			if (results.length > 0) {
				...
				jQuery.each(results, function (k, v) {
					...
					if (v.getAttribute("hotline") != "") {
						phoneFax += "<div class='hotline'><span>" + lang[currCulture].hotline + ":</span> " + v.getAttribute("hotline") + "</div>";
					}
					if (v.getAttribute("phoneMaintenance") != "") {
						phoneFax += "<div><span>" + lang[currCulture].phoneMaintenance + ":</span> " + v.getAttribute("phoneMaintenance") + "</div>";
					}
					if (v.getAttribute("fax") != "") {
						phoneFax += "<div><span>" + lang[currCulture].fax + ":</span> " + v.getAttribute("fax") + "</div>";
					}
					if (v.getAttribute("email") != "") {
						phoneFax += "<div><span>Email:</span> " + v.getAttribute("email") + "</div>";
					}
					...
					if (number > 9) {
						$("<li id='l_" + marker['__gm_id'] + "' class='clinic_list double-digit' />")
							.html("<span class='number'>" + number + "</span><div class='title'><h2>" + lang[currCulture].name + v.getAttribute("name") + "</h2><div class='address'><span>" + lang[currCulture].address + ":</span> " + v.getAttribute("address") + "</div>" + phoneFax + "<div id=d_" + number + " class='distance'><span id='disval' attr-dist='" + distancenode + "' class='value'>" + distancenode + "</span> <span class='units'></span> <span class='time2'></span></div></div><div class='products'>" + ctype + "</div>")
							.click(function () {
								//displayPoint(marker, i);
								infowindow.setContent(info_window_string);
								infowindow.open(map, marker);
