...
function scrollNav(){
	$('.canhcam-nav-4 ul.nav , .canhcam-nav-1 ul.nav , .canhcam-nav-3 ul.nav').hide();
	$('.canhcam-nav-4 .trigger-btn , .canhcam-nav-1 .trigger-btn , .canhcam-nav-3 .trigger-btn').on('click', function() {
		if ($('.canhcam-nav-4 nav , .canhcam-nav-1 nav , .canhcam-nav-3 nav').is(':hidden') === true) {
			$('.canhcam-nav-4 nav , .canhcam-nav-1 nav , .canhcam-nav-3 nav').slideDown('slow')
			$(this).addClass('active')
		} else {
			$('.canhcam-nav-4 nav , .canhcam-nav-1 nav , .canhcam-nav-3 nav').slideUp('slow')
			$(this).removeClass('active')
		}

	})
	$('.canhcam-nav-4 ul.nav , .canhcam-nav-1 ul.nav , .canhcam-nav-3 ul.nav').removeAttr( "style" )
}
