...
function checkMenuScroll() {
	...
	if (((n - m) - e) <= 0) {
		...
		$('.canhcam-nav-3').removeClass('active').removeAttr('style')
	}

	$('.canhcam-nav-3 nav li').each(function() {
		var u = $(this).find('a').attr('data-link')
		var s = $('#' + u).offset().top
		if (((l + e) - 400) >= s) {
			$('.canhcam-nav-3 nav li a').removeClass('active')
